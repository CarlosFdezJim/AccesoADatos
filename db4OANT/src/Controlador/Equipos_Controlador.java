...
public class Equipos_Controlador implements ActionListener{
	...
	ObjectContainer db = Db4oEmbedded.openFile(Db4oEmbedded.newConfiguration(), DataBase);
        ...
        ObjectSet<Equipo> result = db.queryByExample(new Equipo(teamArrayList.get(fila).getIDequipo(), null,null,null,null));
	
        Equipo existe = (Equipo) result.next();     
        //modificar los datos
        existe.setIDequipo(this.MiVista.txtID_Equipos.getText());
        existe.setNombre_eq(this.MiVista.txtNombre_Equipos.getText());
        existe.setLiga(this.MiVista.txtLiga_Equipos.getText());
        existe.setNumJugadores(this.MiVista.txtNumJugadores_Equipos.getText());
        existe.setPresupuesto(this.MiVista.txtPresupuesto_Equipos.getText());
        db.store(existe);
	...
	ObjectContainer db = Db4oEmbedded.openFile(Db4oEmbedded.newConfiguration(), DataBase);
        ...
        if(encontrado == false){
        ObjectSet<Equipo> result = db.queryByExample(new Equipo(this.teamArrayList.get(fila).getIDequipo(), null, null, null, null));
                while (result.hasNext()) {
                    Equipo eq = result.next();
                    db.delete(eq);
    ...
    public void cargarDataBaseOLeerFichero(boolean crear) throws IOException, FileNotFoundException, ClassNotFoundException, SAXException, ParserConfigurationException{
       ...
       public void insertarDatosEnBD(){
        ...
        for(int i = 0; i < teamArrayList.size();i++){
            db.store(this.teamArrayList.get(i));
        }
    ...
    public void Consulta(){
        ...
        Equipo eq = new Equipo(null,null,null,null,null);
        ObjectSet<Equipo> result = db.queryByExample(eq);
        
        if (result.size() == 0)
                System.out.println("No existen Equipos..");
        else {
                System.out.printf("Número de Equipos: %d %n", result.size());

                while (result.hasNext()) {
                        Equipo e = result.next();
                        teamArrayList.add(e);
                }
        }
